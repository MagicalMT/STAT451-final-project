...
# Calculate the count and percentage of each operating system within each age group
os_counts <- data %>%
...
# Create pie chart with percentage labels for each age group
...
ggplot(data, aes(x = Age_Group, y = App.Usage.Time..min.day.)) +
  geom_boxplot(fill = "lightblue", color = "darkblue") + # Box plot with colors
...
### Question5: Is the use of different device models operating systems correlated with the length of screen usage? Or the number of apps installed?

```{r}
correlation_data <- data[, c("Screen.On.Time..hours.day.", 
...
ggcorrplot(correlation_matrix, 
           ...
           title = "Correlation Matrix of Screen Time and Device Variables", 
           colors = c("blue", "white", "red")) # Blue for negative, red for positive correlations

ggplot(data, aes(x = Operating.System, y = App.Usage.Time..min.day., fill = Operating.System)) +
  geom_boxplot() +
  labs(title = "App Usage Time by Operating System", 
       x = "Operating System", 
       y = "Daily App Usage Time (minutes)") +
  theme_minimal()
```

### Question 6: Is this is not a fake dataset
```{r}
ggplot(data, aes(x = App.Usage.Time..min.day.)) +
  geom_histogram(binwidth = 30, fill = "lightblue", color = "darkblue") + # Set bin size and colors
